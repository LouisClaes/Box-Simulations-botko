...
class PalletResult:
    ...
    rejected_boxes: int = 0  # Boxes rejected while this pallet was active (duo metric)
...
class PackingSession:
    ...
    **Smart Pallet Close Logic:**

    Pallets close when stuck (preventing wasted boxes):

      1. **LOOKAHEAD CONDITION**: If current pick window (4 boxes) + next 4 boxes
         in buffer ALL cannot fit on ANY pallet → close fullest pallet
         - Prevents rejecting 8+ boxes to find one placement
         - FIFO principle: rejecting 1 box makes next box grippable

      2. **MAX REJECTS LIMIT**: If 8+ boxes rejected cumulatively → force close
         - Prevents infinite cycling (e.g., every 5th box fits, resets lookahead)
         - Hard cap on wasted boxes per pallet pair

      3. **SAFETY CHECKS**:
         - Never close last active pallet (always keep 1 open)
         - Only close pallets >=50% full (prevents closing empty pallets)
         - Close fullest pallet when multiple qualify

      4. **RESET CONDITIONS**:
         - Counter resets when: box placed successfully OR pallet closed
         - After close: try same boxes on remaining pallet (no advance)
    """
    ...
    def step(
        ...
        if result is not None:
            ...
            pallet_closed = False
            closed_result = None
            # Add global consecutive rejects to stats for close policy
            stats_with_global = {**station.pallet_stats, "global_consecutive_rejects": self._consecutive_rejects}
            if self._config.close_policy.should_close(station.bin_state, stats_with_global):
            ...
            # Check if any pallet should close after rejection (e.g., stuck pallets)
            # Find the fullest pallet that should close based on policy
            # Add global consecutive rejects to stats for all stations
            stations_to_close = [
                st for st in self._stations
                if self._config.close_policy.should_close(
                    st.bin_state,
                    {**st.pallet_stats, "global_consecutive_rejects": self._consecutive_rejects}
                )
            ]
            ...
            if stations_to_close:
                # Close the pallet with highest fill factor
                fullest = max(stations_to_close, key=lambda st: st.bin_state.get_fill_rate())
                closed_result = fullest.snapshot(self._config.bin_config, rejected_boxes=self._consecutive_rejects)
                self._closed.append(closed_result)
                fullest.reset(PipelineSimulator(self._exp_config))
                self._consecutive_rejects = 0
                pallet_closed = True
    ...
    def result(self) -> SessionResult:
        ...
        for st in self._stations:
            if st.boxes_placed > 0:
                # Active pallets get the current duo's reject count
    ...
    def _run_singlebin_step(
        ...
        if best_box is not None and best_decision is not None:
            ...
            # ═══════════════════════════════════════════════════════════════════
            # NO PLACEMENT FOUND - SMART CLOSE LOGIC
            # ═══════════════════════════════════════════════════════════════════
            # Two conditions trigger pallet closure:
            # 1. LOOKAHEAD: Current 4 + next 4 boxes don't fit (would reject 8 boxes)
            ...
            obs = self.observe()
            buffer_boxes = list(obs.buffer_view) if obs.buffer_view else []

            # Current pick window already tested (sorted_boxes) - none fit
            # Check NEXT 4 boxes (boxes 4-7 in buffer, if they exist)
            next_4_boxes = buffer_boxes[len(sorted_boxes):len(sorted_boxes)+4] if len(buffer_boxes) > len(sorted_boxes) else []

            # Test if ANY of the next 4 boxes would fit on ANY pallet
            next_4_has_placement = False
            if next_4_boxes:
                for box in next_4_boxes:
                    for station in self._stations:
                        state = station.bin_state
                        decision = strategy.decide_placement(box, state)
                        if decision is not None:
                            next_4_has_placement = True
                            break
                    if next_4_has_placement:
                        break

            # Lookahead condition: NEITHER current 4 NOR next 4 have placements
            lookahead_stuck = (
                not next_4_has_placement
                and len(sorted_boxes) >= 4
                and len(next_4_boxes) >= 4
            )
            ...
            # Safety check: only close if we have multiple active pallets
            active_pallets = [st for st in self._stations if st.boxes_placed > 0]
            can_close = len(active_pallets) > 1  # Never close last pallet
            ...
            if stations_to_close:
                # Close the fullest pallet
                fullest = max(stations_to_close, key=lambda st: st.bin_state.get_fill_rate())
                closed_result = fullest.snapshot(self._config.bin_config, rejected_boxes=self._consecutive_rejects)

                # Detailed logging with close reason
                close_reason = "MAX REJECTS (8+)" if max_rejects_reached else "LOOKAHEAD (current+next 4)"
                print(f"  [PALLET CLOSED] Station {fullest.slot_index}: {fullest.boxes_placed} boxes, {fullest.bin_state.get_fill_rate():.1%} fill")
                print(f"    Reason: {close_reason} | Total rejects: {self._consecutive_rejects}")

                self._closed.append(closed_result)
                fullest.reset(PipelineSimulator(self._exp_config))
                self._consecutive_rejects = 0  # Reset counter after close

                # After closing, try again with remaining pallet
                # (conveyor doesn't advance - same boxes available next step)
                return StepResult(
                    placed=False, placement=None,
                    box=None, bin_index=-1,
                    pallet_closed=True,
                    closed_pallet_result=closed_result,
                )

            # ── NO PALLET CLOSED ──────────────────────────────────────────────
            # Advance conveyor (reject front box) and continue
    ...
    def _check_done(self) -> None:
        ...
        1. No conveyor (initialization issue)
        2. Conveyor is empty (all boxes processed)
        3. Max consecutive rejects hit (safety valve)
        4. OPTIMIZATION: Not enough boxes to fill another pallet (early termination)
        """
        conv = self._conveyor
        if conv is None:
            self._done = True
        ...
        # If we have closed pallets and remaining boxes can't reach 50% fill
        # threshold, terminate early to save simulation time.
        # Only closed pallets count in metrics, so simulating an unfillable
        # pallet wastes time.
        # ──────────────────────────────────────────────────────────────────────
        if len(self._closed) > 0:  # Have at least one closed pallet (have data)
            remaining_boxes = conv.total_remaining

            # Calculate minimum boxes placed in any closed pallet
            min_boxes_per_pallet = min(p.boxes_placed for p in self._closed)

            # Conservative threshold: half of minimum boxes per pallet
            # (assumes average box size, won't reach 50% fill threshold)
            early_term_threshold = max(10, min_boxes_per_pallet // 2)

            # Also check if active pallets are very empty (< 30% full)
            ...
            # Terminate if:
            # 1. Remaining boxes < threshold
            # 2. All active pallets are below 30% (unlikely to reach 50%)
            if remaining_boxes < early_term_threshold and max_active_fill < 0.3:
                ...
                print(f"    Max active fill: {max_active_fill:.1%} (below 30% threshold)")
                print(f"    Closed pallets: {len(self._closed)} (will be used in metrics)")
                self._done = True
                return
